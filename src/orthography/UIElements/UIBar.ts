...
const UIBar = (data: IAlert, loading: boolean, showDictionary: boolean = false, dictionary: string[] = []): string => {
  ...
  const cards = showDictionary ? (UIDictionary(dictionary)) : (hasData ? UIHints(data.alerts) : fallback);
